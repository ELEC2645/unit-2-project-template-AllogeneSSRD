...
void buck_ccm_inductor_Iripple(void) {
    ...
    input_float(&Vo, "Output Voltage (Vo)");
    input_float(&K,  "Duty Cycle (K)");
    input_float(&L,  "Inductance (L)");
    input_float(&f_s, "Switching Frequency (f_s)");
    input_float(&delta_i, "Current Ripple (Delta i)");
    // Determine if more than one variable is unknown
    ...
    if (isnan(delta_i)) {
        ...
        printf("Calculated Current Ripple: Delta i = ((1 - K) * Vo) / (f_s * L): %.4f\n", delta_i);
    } else if (isnan(Vo)) {
        Vo = (delta_i * f_s * L) / (1 - K);
        printf("Calculated Output Voltage: Vo = (Delta i * f_s * L) / (1 - K): %.4f\n", Vo);
    } else if (isnan(K)) {
        K = 1 - (delta_i * f_s * L) / Vo;
        printf("Calculated Duty Cycle: K = 1 - (Delta i * f_s * L) / Vo: %.4f\n", K);
    } else if (isnan(L)) {
        L = ((1 - K) * Vo) / (f_s * delta_i);
        printf("Calculated Inductance: L = ((1 - K) * Vo) / (f_s * Delta i): %.4f\n", L);
    } else if (isnan(f_s)) {
        f_s = ((1 - K) * Vo) / (L * delta_i);
        printf("Calculated Switching Frequency: f_s = ((1 - K) * Vo) / (L * Delta i): %.4f\n", f_s);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
