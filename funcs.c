...
} Converter_Params;    // Parameters = Variables (show to user)
...
void buck_ccm_inductor_Iripple(void)
{
    ...
    if (isnan(Delta_i)) {
        ...
        output_float("Calculated Current Ripple (Delta i = ((1 - K) * Vo) / (f_s * L)): Delta i", Delta_i, "A", FALSE, 0, 0);
    } else if (isnan(Vo)) {
        Vo = (Delta_i * f_s * L) / (1 - K);
        output_float("Calculated Output Voltage (Vo = (Delta i * f_s * L) / (1 - K)): Vo", Vo, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
        K = 1 - (Delta_i * f_s * L) / Vo;
        output_float("Calculated Duty Cycle (K = 1 - (Delta i * f_s * L) / Vo): K", K, "", TRUE, 0, 1);
    } else if (isnan(L)) { // L = \frac{V_{o} (1 - K)}{\Delta_{i} f_{s}}
        L = ((1 - K) * Vo) / (f_s * Delta_i) * 1e3; // convert to mH
        output_float("Calculated Inductance (L = ((1 - K) * Vo) / (f_s * Delta i)): L", L, "mH", FALSE, 0, 0);
    } else if (isnan(f_s)) {
        f_s = ((1 - K) * Vo) / (L * Delta_i);
        output_float("Calculated Switching Frequency (f_s = ((1 - K) * Vo) / (L * Delta i)): f_s", f_s, "Hz", FALSE, 0, 0);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void buck_ccm_capacitor_Vripple(void)
{
    ...
    if (isnan(Delta_v)) {
        ...
        output_float("Calculated Voltage Ripple (Delta v = ((1 - K) * Vo) / (8 * f_s^2 * C * L)): Delta v", Delta_v, "V", FALSE, 0, 0);
    } else if (isnan(Vo)) {
        Vo = (Delta_v * 8 * f_s * f_s * C * L) / (1 - K);
        output_float("Calculated Output Voltage (Vo = (Delta v * 8 * f_s^2 * C * L) / (1 - K)): Vo", Vo, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
        K = 1 - (Delta_v * 8 * f_s * f_s * C * L) / Vo;
        output_float("Calculated Duty Cycle (K = 1 - (Delta v * 8 * f_s^2 * C * L) / Vo): K", K, "", TRUE, 0, 1);
    } else if (isnan(C)) {
        C = ((1 - K) * Vo) / (Delta_v * 8 * f_s * f_s * L) * 1e6; // convert to uF
        output_float("Calculated Capacitance (C = ((1 - K) * Vo) / (Delta v * 8 * f_s^2 * L)): C", C, "uF", FALSE, 0, 0);
    }else if (isnan(L)) {
        L = ((1 - K) * Vo) / (Delta_v * 8 * f_s * f_s * C) * 1e3; // convert to mH
        output_float("Calculated Inductance (L = ((1 - K) * Vo) / (Delta v * 8 * f_s^2 * C)): L", L, "mH", FALSE, 0, 0);
    } else if (isnan(f_s)) {
        f_s = ((1 - K) * Vo) / (8 * C * Delta_v);
        output_float("Calculated Switching Frequency (f_s = ((1 - K) * Vo) / (8 * C * Delta v)): f_s", f_s, "Hz", FALSE, 0, 0);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void buck_dcm_duty_cycle(void)
{
    ...
    if (isnan(Vo)) {
        ...
        output_float("Calculated Output Voltage (Vo = Vi * K / (K + Delta 1)): Vo", Vo, "V", FALSE, 0, 0);
    } else if (isnan(Vi)) {
        Vi = Vo * (K + Delta_1) / K; // Vi = \frac{V_{o} (K + \Delta_{i})}{K}
        output_float("Calculated Input Voltage (Vi = Vo * (K + Delta 1) / K): Vi", Vi, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
        K = (Vo * Delta_1) / (Vi - Vo); // K = \frac{\Delta_{i} V_{o}}{V_{i} - V_{o}}
        output_float("Calculated Duty Cycle (K = (Vo * Delta 1) / (Vi - Vo)): K", K, "", TRUE, 0, 1);
    } else if (isnan(Delta_1)) {
        Delta_1 = (K * (Vi - Vo)) / Vo; // Delta_i = \frac{K (V_{i} - V_{o})}{V_{o}}
        output_float("Calculated Close time (Delta 1 = (K * (Vi - Vo)) / Vo): Delta 1", Delta_1, "", TRUE, 0, 1);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void buck_dcm_boundary_constant_vi(void)
{
    ...
    if (isnan(Vi)) {
        ...
        output_float("Calculated Input Voltage (Vi = -2 * Io * L* f_s / (K * (K - 1))): Vi", Vi, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
        K = (Vi - sqrt(Vi * (-8 * Io * L * f_s + Vi)))/(2 * Vi); // K = \frac{V_{i} - \sqrt{V_{i} (- 8 I_{o} L f_{s} + V_{i})}}{2 V_{i}}
        output_float("Calculated Duty Cycle (K = (Vi - sqrt(Vi * (-8 * Io * L * f_s + Vi)))/(2 * Vi)): K", K, "", TRUE, 0, 1);
    } else if (isnan(L)) {
        L = (Vi * K * (1 - K)) / (2 * Io * f_s) * 1e3; // L = \frac{V_{i} K{(1 - K )}}{2 I_{o} f_{s}}
        output_float("Calculated Inductance (L = (Vi * K * (1 - K)) / (2 * Io * f_s)): L", L, "mH", FALSE, 0, 0);
    } else if (isnan(f_s)) {
        f_s = K * Vi * (1 - K)/(2 * Io * L); // f_{s} = \frac{K V_{i} (1 - K)}{2 I_{o} L}
        output_float("Calculated Switching Frequency (f_s = K * Vi * (1 - K)/(2 * Io * L)): f_s", f_s, "Hz", FALSE, 0, 0);
    } else if (isnan(Io)) {
        Io = Vi * K * (1 - K)/(2 * L * f_s); // I_{o} = \frac{V_{i} K{(1 - K )}}{2 L f_{s}}
        output_float("Calculated Output Current (Io = Vi * K * (1 - K)/(2 * L * f_s)): Io", Io, "A", FALSE, 0, 0);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void buck_dcm_boundary_constant_vo(void)
{
    ...
    if (isnan(Vo)) {
        ...
        output_float("Calculated Input Voltage (Vo = 2 * Io * L * f_s / (1 - K)): Vo", Vo, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
        K = 1 - (2 * Io * L * f_s)/Vo; // K = \frac{- 2 I_{o} L f_{s}}{V_{o}} + 1
        output_float("Calculated Duty Cycle (K = 1 - (2 * Io * L * f_s)/Vo): K", K, "", TRUE, 0, 1);
    } else if (isnan(L)) {
        L = Vo * (1 - K)/(2 * Io * f_s) * 1e3; // L = \frac{V_{o} \left(1 - K\right)}{2 I_{o} f_{s}}
        output_float("Calculated Inductance (L = Vo * (1 - K)/(2 * Io * f_s)): L", L, "mH", FALSE, 0, 0);
    } else if (isnan(f_s)) {
        f_s = Vo * (1 - K)/(2 * Io * L); // f_{s} = \frac{V_{o} \left(1 - K\right)}{2 I_{o} L}
        output_float("Calculated Switching Frequency (f_s = Vo * (1 - K)/(2 * Io * L)): f_s", f_s, "Hz", FALSE, 0, 0);
    } else if (isnan(Io)) {
        Io = Vo * (1 - K)/(2 * L * f_s); // I_{o} = \frac{V_{o} \left(1 - K\right)}{2 L f_{s}}
        output_float("Calculated Output Current (Io = Vo * (1 - K)/(2 * L * f_s)): Io", Io, "A", FALSE, 0, 0);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void boost_ccm_duty_cycle(void)
{
    ...
    if (isnan(Vo)) {
        ...
        output_float("Calculated Input Voltage (Vo = Vi/(1 - K)): Vo", Vo, "V", FALSE, 0, 0);
    } else if (isnan(Vi)) {
        Vi = Vo * (1 - K); // Vi = V_o \left(1 - K\right)
        output_float("Calculated Input Voltage (Vi = Vo * (1 - K)): Vi", Vi, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
        K = 1- Vi/Vo; // K = 1-\frac{V_i}{V_o}
        output_float("Calculated Duty Cycle (K = 1 - Vi/Vo): K", K, "", TRUE, 0, 1);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void boost_ccm_inductor_Iripple(void)
{
    ...
    if (isnan(Vi)) {
        ...
        output_float("Calculated Input Voltage (Vi = Delta_i * L * f_s / K): Vi", Vi, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
        K = Delta_i * L * f_s/Vi; // K = \frac{\Delta_{i} L f_{s}}{V_{i}}
        output_float("Calculated Duty Cycle (K = Delta_i * L * f_s / Vi): K", K, "", TRUE, 0, 1);
    } else if (isnan(L)) {
        L = K * Vi / (Delta_i * f_s) * 1e-3; // L = \frac{K V_{i}}{\Delta_{i} f_{s}}
        output_float("Calculated Inductance (L = K * Vi / (Delta_i * f_s)): L", L, "H", FALSE, 0, 0);
    } else if (isnan(f_s)) {
        f_s = K * Vi/(Delta_i * L); // f_{s} = \frac{K V_{i}}{\Delta_{i} L}
        output_float("Calculated Switching Frequency (f_s = K * Vi / (Delta_i * L)): f_s", f_s, "Hz", FALSE, 0, 0);
    } else if (isnan(Delta_i)) {
        Delta_i = K * Vi/(L * f_s); // \Delta_{i} = \frac{K V_{i}}{L f_{s}}
        output_float("Calculated Inductor Current Ripple (Delta_i = K * Vi / (L * f_s)): Delta_i", Delta_i, "A", FALSE, 0, 0);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void boost_ccm_capacitor_Vripple(void)
{
    ...
    if (isnan(Vo)) {
        ...
        output_float("Calculated Output Voltage (Vo = (Delta_v * R * f_s * C)/(K)): Vo", Vo, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
        K = C * Delta_v * R * f_s / Vo; // K = \frac{C \Delta_{v} R f_{s}}{V_{o}}
        output_float("Calculated Duty Cycle (K = C * Delta_v * R * f_s / Vo): K", K, "", TRUE, 0, 1);
    } else if (isnan(R)) {
        R = K * Vo / (C * Delta_v * f_s); // R = \frac{K V_{o}}{C \Delta_{v} f_{s}}
        output_float("Calculated Load Resistance (R = K * Vo / (C * Delta_v * f_s)): R", R, "Ohm", FALSE, 0, 0);
    } else if (isnan(C)) {
        C = K * Vo / (Delta_v * R * f_s)  *  1e6; // C = \frac{K V_{o}}{\Delta_{v} R f_{s}}
        output_float("Calculated Capacitance (C = K * Vo / (Delta_v * R * f_s)): C", C, "uF", FALSE, 0, 0);
    } else if (isnan(f_s)) {
        f_s = K * Vo / (C * Delta_v * R); // f_{s} = \frac{K V_{o}}{C \Delta_{v} R}
        output_float("Calculated Switching Frequency (f_s = K * Vo / (C * Delta_v * R)): f_s", f_s, "Hz", FALSE, 0, 0);
    } else if (isnan(Delta_v)) {
        Delta_v = K * Vo / (C * R * f_s); // \Delta_{v} = \frac{K V_{o}}{C R f_{s}}
        output_float("Calculated Capacitor Voltage Ripple (Delta_v = K * Vo / (C * R * f_s)): Delta_v", Delta_v, "V", FALSE, 0, 0);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void boost_dcm_duty_cycle(void)
{
    ...
    if (isnan(Vo)) {
        ...
        output_float("Calculated Output Voltage (Vo = Vi * (Delta_1 + K) / Delta_1): Vo", Vo, "V", FALSE, 0, 0);
    } else if (isnan(Vi)) {
        Vi = Delta_1 * Vo / (Delta_1 + K); // V_{i} = \frac{\Delta_{1} V_{o}}{\Delta_{1} + K}
        output_float("Calculated Input Voltage (Vi = Delta_1 * Vo / (Delta_1 + K)): Vi", Vi, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
        K = Delta_1 * (-Vi + Vo) / Vi; // K = \frac{\Delta_{1} \left(- V_{i} + V_{o}\right)}{V_{i}}
        output_float("Calculated Duty Cycle (K = Delta_1 * (-Vi + Vo) / Vi): K", K, "", TRUE, 0, 1);
    } else if (isnan(Delta_1)) {
        Delta_1 = -K * Vi / (Vi - Vo); // \Delta_{1} = - \frac{K V_{i}}{V_{i} - V_{o}}
        output_float("Calculated Close time (Delta_1 = -K * Vi / (Vi - Vo)): Delta_1", Delta_1, "", FALSE, 0, 0);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void boost_dcm_boundary_constant_vi(void)
{
    ...
    if (isnan(Vi)) {
        ...
        output_float("Calculated Inductance (L = K*Vi*(1 - K)/(2*Io*f_s)): L", L, "mH", FALSE, 0, 0);
    } else if (isnan(f_s)) {
        f_s = K*Vi*(1 - K)/(2*Io*L); // f_{s} = \frac{K V_{i} \left(1 - K\right)}{2 I_{o} L}
        output_float("Calculated Switching Frequency (f_s = K*Vi*(1 - K)/(2*Io*L)): f_s", f_s, "Hz", FALSE, 0, 0);
    } else if (isnan(Io)) {
        Io = K*Vi*(1 - K)/(2*L*f_s); // I_{o} = \frac{K V_{i} \left(1 - K\right)}{2 L f_{s}}
        output_float("Calculated Output Current (Io = K*Vi*(1 - K)/(2*L*f_s)): Io", Io, "A", FALSE, 0, 0);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
...
void boost_dcm_boundary_constant_vo(void)
{
    ...
    if (isnan(Vo)) {
        ...
        output_float("Calculated Input Voltage (Vo = 2*Io*L*f_s/(K*(K - 1)^2)): Vo", Vo, "V", FALSE, 0, 0);
    } else if (isnan(K)) {
    /*
    ...
    */
        ...
        output_float("Calculated Inductance (L = K*Vo*(K - 1)*(K - 1)*2/(2*Io*f_s)): L", L, "mH", FALSE, 0, 0);
    } else if (isnan(f_s)) {
        f_s = K*Vo*(K - 1)*(K - 1)*2/(2*Io*L); // f_{s} = \frac{K V_{o} \left(K - 1\right)^{2}}{2 I_{o} L}
        output_float("Calculated Switching Frequency (f_s = K*Vo*(K - 1)*(K - 1)*2/(2*Io*L)): f_s", f_s, "Hz", FALSE, 0, 0);
    } else if (isnan(Io)) {
        Io = K*Vo*(K - 1)*(K - 1)*2/(2*L*f_s); // I_{o} = \frac{K V_{o} \left(K - 1\right)^{2}}{2 L f_{s}}
        output_float("Calculated Output Current (Io = K*Vo*(K - 1)*(K - 1)*2/(2*L*f_s)): Io", Io, "A", FALSE, 0, 0);
    } else {
        printf("All variables provided. No calculation needed.\n");
    }
    // Save variables to file
